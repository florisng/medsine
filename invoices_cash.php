	...
	<body>
		...
		<div id="welcome">
			...
			<div id="entete">
								...
								<span class="titre">Invoices - Cash 100%</span>
...
<option value="<?php echo $year; ?>" <?php if(isset($_GET["year"])){if($_GET["year"] == $year){echo "selected";}}else{if($current_year == $year){echo "selected";}}?>><?php echo $year; ?></option>
			...
			<option value="All" <?php if(isset($_GET["pay_mode"])){if($_GET["pay_mode"] == "All"){echo "selected";}} ?>>All</option>
			<option value="CASH" <?php if(isset($_GET["pay_mode"])){if($_GET["pay_mode"] == "CASH"){echo "selected";}} ?>>CASH</option>
			<option value="MoMo" <?php if(isset($_GET["pay_mode"])){if($_GET["pay_mode"] == "MoMo"){echo "selected";}} ?>>MoMo</option>
			<option value="POS" <?php if(isset($_GET["pay_mode"])){if($_GET["pay_mode"] == "POS"){echo "selected";}} ?>>POS</option>
												</select>
											</td>
											<td>Cashier:</td>
											<td>
					...
					<option value="<?php echo $data["username"]; ?>" <?php if(isset($_GET["username"])){if($_GET["username"] == $data["username"]){echo "selected";}}?>><?php echo $data["prenom"]; ?></option>
												...
												<?php
													if(isset($_GET["month"])) {
														echo "<a href='invoices_cash.php' class='link'><<< Back</a>";
													}
												?>
											</td>
										</tr>
			...
			echo "<img src='img/warning.png' class='small' /> <b>Error !!!</b> The invoice num <b>".$_POST["num_fact"]."</b> does NOT exist !!!";
					...
					if(isset($_POST["del_fact"]))
					{
							echo"<div id='okMsg'>";
								$bdd = connexionDb();
								$bdd->query("DELETE FROM facture WHERE numero = '".$_POST["num_fact"]."'");
					...
					if($nbr <= 0)
					{
						...
						if(isset($_GET["month"]) AND isset($_GET["year"]) AND isset($_GET["pay_mode"]) AND isset($_GET["username"]))
						{
							if($_GET["month"] < 0 OR $_GET["month"] > 12) {
								...
								if($_GET["year"] > $current_year) {
									...
									$f = new Facture();
									$f->search_invoices_cash($_GET["month"], $_GET["year"], $_GET["pay_mode"], $_GET["username"]);
								}
							}
						} else {
							$f = new Facture();
